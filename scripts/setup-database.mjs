...
async function setupDatabase() {
  try {
    ...
    const { error: beritaError } = await supabase.from('berita').select().limit(0);
    ...
    const { error: galeriError } = await supabase.from('galeri').select().limit(0);
    ...
    console.log('Membuat bucket storage...');
    ...
    if (!buckets?.find(b => b.name === 'media')) {
      ...
      if (error) throw error;
      console.log('Bucket media berhasil dibuat!');
    }
